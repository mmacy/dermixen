...
Four files in that directory are headers rather than music, and they are what the decoder's limits are tested against. `silence-91-minutes.flac` and `silence-91-minutes-no-total.flac` are each 64 KB that decode to 91 minutes of digital silence, which is past the 90 minutes a track may be. The first states its length in its header and the second states none, so the decoder refuses the first before it decodes anything and refuses the second as soon as its packets pass 90 minutes. `tools/make_long_flac.py` writes both. `wav-65535-channels.wav` states 65,535 channels and `m4a-huge-sample-count.m4a` states 4,294,967,295 samples in its `stts` box, and each number reaches arithmetic in symphonia 0.6.1 that overflows, so a decode of either has to come back as an error rather than end the process.
