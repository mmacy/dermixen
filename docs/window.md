...
The window reads a mix document of at most 16 MB, and it reads a regular file only. A path that names a device, a named pipe, a folder, or a symbolic link to any of those is refused with one line saying that the path is not a regular file, and so is a file over that size, with the limit in bytes in the line. The same rule holds wherever the window reads a document: the file you name on the command line, the file **Open...** gives it, a `.dmx` file you open on the desktop, and an autosave file. A link to `/dev/zero` named `set.dmx` and double-clicked therefore says one sentence rather than taking the machine's memory.
...
A track the library can place is pointed at the file the library names, the mix counts as changed so that a save keeps the new path, and the status line says which tracks were relinked. An edit you make while the pass runs outranks a path it found: the window keeps your edit, points no track at a new file, and the status line says to open the mix again to point them. A pass that found no new file for any track leaves the document, the history, and the beat grid editor as they are. The window points the library's record for that track at that file as well, so the window opens the library file for writing while the pass runs. A `dermixen library scan` running in another process at the same time keeps the library file to itself, and the pass then stops, with the reason SQLite reported in the status line and a note that not every track was looked for.
...
The window compares what it decoded with what the document names. Every track's file is hashed as it is decoded, and a file whose hash is not the one the document gives for that track is another recording, or the same recording encoded again. The window treats such a file as it treats a missing one: the lane says `the file has changed` in place of the waveform, the audio that was decoded is let go of rather than played, and the status line says that the file at that path is not the track the document names and points at `dermixen mix relink`. That is the answer whether the file changed between the pass above and the reading, or the beat grid editor or the preview reached the file first.
...
An autosave file is read under the same rule as a mix document: a regular file of at most 16 MB. A link to a device planted at an autosave's name is reported as a file that could not be read rather than read without end.
