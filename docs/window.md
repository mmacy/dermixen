...
`dermixen-app --help` prints the usage and exits with code 0. The window produces three exit codes: 0 when it opened and closed, 1 when the mix could not be read or is not a mix document, with one line beginning `error:` on standard error saying which, and 2 when the command line could not be read, which is more than one argument. Code 3, which `docs/cli.md` describes for the `dermixen` command, belongs to that command alone: the window never produces it. The window also exits with code 1 when it cannot read the settings file. That `error:` line names the settings file and what is wrong, and the line the problem is on when the file has one, which a file the operating system refuses to read does not. The window reads the settings file before it opens on the mix, and `docs/settings.md` describes the file.
