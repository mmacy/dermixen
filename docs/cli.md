...
| 3 | The command met a defect in dermixen itself. One line beginning `error:` on standard error names the file and line the defect happened at and what it said. Nothing the command had not already written is written. A command that ends this way is worth reporting. |
...
No writing command replaces one of its own inputs. `render` and `play --capture` refuse an output that is the mix document or a track of the mix, and `decode` refuses one that is the file being decoded. The refusal holds whether the output names the file directly, through `..`, through a symbolic link, or through a hard link, and it names the file. Each of the three replaces an output that is not one of its inputs, since a render is made again from its document. `mix new` refuses to replace a document that is already there, since a document is work.
