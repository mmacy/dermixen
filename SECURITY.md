...
The `dermixen` command and the `dermixen-app` window read audio files, mix documents and their autosaves, the settings file, the library file, a playlist for `mix plan`, and a folder to scan. Every one of these has a stated limit. The "Limits" section of `DESIGN.md` gives every number: the size and sample rate a decoder accepts, the length of a mix or a track, the size of a mix document or a playlist, and the rest.
...
The shipped `dermixen` command and `dermixen-app` window open no network connection. `cargo tree --workspace -e normal | grep -iE 'reqwest|hyper|ureq|rustls|native-tls|openssl|curl|tokio'` prints nothing, confirming that no crate in the workspace depends on a networking or TLS library.
